namespace Pooshit.AudioSynth.Synthesis {
    ...
    /// the oscillation frequency, and the peak pitch deviation it drives at full excursion.  SF2
    /// timecent and absolute-cent units are converted away before an instance is built, so this type
    /// carries no SoundFont-specific knowledge.  <see cref="Default"/> is inert (zero pitch depth), so
    /// building a <see cref="ModulationLfo"/> from it is an exact passthrough at any rate.
    /// </summary>
    public readonly struct LfoParameters {
        ...
        public LfoParameters(float delaySeconds, float frequencyHz, float pitchDepthCents) {
        ...
        /// timecents), the default frequency (8.176 Hz, from 0 absolute cents), and zero pitch depth.
        /// Zero depth makes the LFO inert regardless of delay or frequency, so hand-built patches and
        /// any region whose SF2 mod-LFO generators are absent render unaffected by the LFO.
        /// </summary>
        public static LfoParameters Default =>
            new LfoParameters(Sf2DefaultDelaySeconds, Sf2DefaultFrequencyHz, 0f);
