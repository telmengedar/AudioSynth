...
Single linear pool scan, applying **two independent conditions per slot** (corrected post-merge, task #7249/W1 — see the update note at the end of this section): for each occupied slot, if `slot.Channel == channel` call `slot.Voice.FastFadeForSteal()` (retarget its `GainRamp` to 0 over the existing ~5ms declick); **separately**, if `slot.PendingChannel == channel` set `slot.PendingChannel = NoPendingNote` (cancel a note parked behind *any* slot's steal fade that is itself destined for `channel`, regardless of which channel's voice is fading in that slot). A slot may match one condition, both, or neither. **Does not** consult `channelSustain`, **does not** call `Release()`. The slot is reclaimed by `Read`'s existing finished-voice branch once the fade reaches silence. This reuses the exact declick path already proven by voice-stealing (INV-1, click-free).

> **Update (task #7249, W1, post-merge correction):** the original text of this section (and the code as first merged in PR #27) keyed the pending-cancel on the SAME condition as the fast-fade (`slot.Channel == channel`). That is wrong: during a voice steal, the slot holds the OUTGOING voice's channel in `slot.Channel` while the PARKED (incoming) note's channel lives in `slot.PendingChannel` — the two can differ. Keying both actions on `slot.Channel` could (a) cancel a parked note bound for a *different* channel than the one being silenced (over-cancel — silent data loss), or (b) fail to cancel a parked note that *is* bound for the silenced channel when it happens to sit behind a different channel's fading victim, letting it resurrect right after All Sound Off (under-cancel — the exact bug §12's "Parked steal-note under CC120" edge case exists to prevent). The fix makes the two checks independent, as written above. See DiVoid design node #7245 (this doc's source) for the historical record and task #7249 / QA #7248 for the diagnosis.
...
|---|---|---|
| `SilenceChannel(int channel)` | Every currently-sounding voice on the channel is fast-faded to silence over the standard declick window; sustain state is ignored; no envelope release runs; a pending steal-note parked *for this channel* is cancelled regardless of which channel's voice is fading in its slot (see §6 update, task #7249/W1). | Bit-identical no-op when the channel has no occupied voices. Other channels untouched. Range-checks `channel` like every other seam. |
...
- **Parked steal-note under CC120:** cancelled (`PendingChannel = NoPendingNote`) so All Sound Off does not resurrect a note that was about to start behind a fading victim — keyed on the parked note's OWN destination channel (`slot.PendingChannel == channel`), independently of which channel's voice is fading in that slot (`slot.Channel`), since a steal can park a note for one channel behind another channel's outgoing voice. See §6's update note (task #7249/W1) for the cross-channel case this independence is required for.
...
- CC121 sustain interaction (end-to-end): pedal down, NoteOn, NoteOff (→ PendingRelease), CC121 → the `SetChannelSustain(false)` step releases the deferred voice. **(Closed by task #7249/W2:** added against a REAL `Synthesizer` through `MidiSequencer.Render`, not only the routing spy — `MidiSequencerHousekeepingControllersTests.Render_Cc121_ThroughRealSynthesizer_ActuallyReleasesSustainDeferredVoice`.)
- **Cross-channel parked-steal cancellation (task #7249/W3, closed):** `SynthesizerSilenceChannelTests` gained two cases constraining the pool to force a cross-channel steal — a note for channel B parked behind channel A's fading victim. `SilenceChannel(A)` must NOT drop B's parked note (over-cancel); `SilenceChannel(B)` must cancel it even though the slot's current voice belongs to A (under-cancel/resurrection). Both reproduce and close the §6/§12 gap.
```
